// app/api/save-data-beneficiaries/route.js
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build query for data beneficiaries
    const { data: beneficiaries, error } = await supabase
      .from('save_data_beneficiaries')
      .select('*')
      .eq('user_id', userId)
    ...
    if (error) {
      console.error('Supabase error fetching data beneficiaries:', error);
      return NextResponse.json(
    ...
    console.error('Error fetching saved data beneficiaries:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!phoneNumber || !network) {
      return NextResponse.json(
        { success: false, message: 'Phone number and network are required' },
        { status: 400 }
      );
    }

    // Check if beneficiary already exists
    const { data: existingBeneficiary, error: existingError } = await supabase
      .from('save_data_beneficiaries')
      .select('*')
      .eq('user_id', userId)
    ...
    if (insertError) {
      console.error('Supabase error inserting data beneficiary:', insertError);
      return NextResponse.json(
    ...
    console.error('Error saving data beneficiary:', error);
    return NextResponse.json(
      { success: false, message: 'Internal server error' },
      { status: 500 }
    );
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updateError) {
      console.error('Supabase error updating data beneficiary:', updateError);
      return NextResponse.json(
    ...
    console.error('Error updating data beneficiary:', error);
    return NextResponse.json(
      { success: false, message: 'Internal server error' },
      { status: 500 }
    );
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Supabase error deleting data beneficiary:', error);
      return NextResponse.json(
    ...
    console.error('Error deleting data beneficiary:', error);
    return NextResponse.json(
      { success: false, message: 'Internal server error' },
      { status: 500 }
    );
