...
export const InvoicePreview = ({ invoice }: InvoicePreviewProps) => {
  ...
  const paymentProgress = invoice.allowMultiplePayments && invoice.targetQuantity
    ...
    <Card className="p-8 bg-invoice-bg border-invoice-border h-full overflow-auto">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-bold text-foreground mb-2">{invoice.businessName}</h2>
          ...
          <div className="inline-block mt-2 px-4 py-1 bg-gold/10 border border-[#C29307] rounded-full">
            <span className="text-sm text-[#C29307] font-semibold">#{invoice.invoiceNumber}</span>
          </div>
        </div>
          ...
          <>
            <div className="mb-6 p-4 bg-muted/50 rounded-lg">
              ...
              <div className="text-foreground">
                {invoice.clientName && <div className="font-semibold">{invoice.clientName}</div>}
              ...
              items.map((item: any) => (
                <div key={item.id} className="flex justify-between items-start p-3 bg-muted/30 rounded-lg">
                  <div className="flex-1">
                    <div className="font-medium text-foreground">{item.description || "Item description"}</div>
        ...
        <div className="p-6 bg-muted/50 rounded-lg border border-border">
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-2xl font-bold">
              ...
              <span className="text-gold">₦{(invoice.total || 0).toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
